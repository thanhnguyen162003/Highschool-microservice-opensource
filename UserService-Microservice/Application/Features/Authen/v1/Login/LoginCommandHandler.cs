...
{
    public class LoginCommandHandler(IUnitOfWork unitOfWork, IAuthenticationService authenticationService,
            IMapper mapper, ICommonTask commonTask) : IRequestHandler<LoginCommand, ResponseModel>
    {
        ...
        public async Task<ResponseModel> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            else if (!_authenticationService.VerifyPasswordHash(request.Password, user.Password!, user.PasswordSalt!))
            {
                return new ResponseModel()
                {
