...
public class NewsEndpoint : ICarterModule
{
    ...
    public static async Task<IResult> CreateNews([FromForm] NewsCreateRequestModel newCreateRequestModel, ISender sender, ValidationHelper<NewsCreateRequestModel> validationHelper, HttpRequest httpRequest)
    {
        newCreateRequestModel.Image = httpRequest.Form.Files.FirstOrDefault();
