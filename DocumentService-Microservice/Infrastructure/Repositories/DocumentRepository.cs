...
namespace Infrastructure.Repositories
{
    public class DocumentRepository(DocumentDbContext context) : BaseRepository<Document>(context), IDocumentRepository
    {
        ...
        public async Task<IEnumerable<string>> GetDocumentBySubjectId(IEnumerable<string> subjectIds, CancellationToken cancellationToken = default)
        {
            var documents = await _entities
                .AsNoTracking()
                .Where(x => x.SubjectCurriculumId != null && subjectIds.Contains(x.SubjectCurriculumId.ToString()))
                .Select(x => x.Id.ToString())
                .ToListAsync();
            return (IEnumerable<string>)documents;
        }

        public async Task<IEnumerable<Document>> GetDocumentByIds(IEnumerable<string> documentIds, CancellationToken cancellationToken = default)
        {
            var documents = await _entities
                .AsNoTracking()
                .Where(x => x.Id != null && documentIds.Contains(x.Id.ToString()))
