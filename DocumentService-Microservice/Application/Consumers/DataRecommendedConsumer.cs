...
public class DataRecommendedConsumer : KafkaConsumerBaseBatch<RecommendedDataModel>
{
    public DataRecommendedConsumer(IConfiguration configuration, ILogger<DataRecommendedConsumer> logger, IServiceProvider serviceProvider)
        : base(configuration, logger, serviceProvider, TopicKafkaConstaints.DataRecommended, "data-recommended-consumer-group")
    {
    }
    ...
    protected override async Task ProcessBatch(IEnumerable<string> messages, IServiceProvider serviceProvider)
    {
        ...
        foreach (var message in messages)
        {
            try
            {
                ...
                List<Guid> flashcardIds = recommendedDataModel.FlashcardIds ?? new();
                List<Guid> documentIds = recommendedDataModel.DocumentIds ?? new();
                ...
                if (existingData is not null)
                {
                    var updatedEntry = new RecommendedData
                    {
                        ...
                        DocumentIds = JsonSerializer.Serialize(documentIds) ?? null,
                        FlashcardIds = JsonSerializer.Serialize(flashcardIds) ?? null,
                        UserId = recommendedDataModel.UserId,
                        Grade = recommendedDataModel.Grade.ToString(),
