...
{
    public class ConsumerUserRoadmapService(
        IConfiguration configuration,
        ILogger<ConsumerUserRoadmapService> logger,
        IServiceProvider serviceProvider)
        : KafkaConsumerBaseBatch<RoadmapUserKafkaMessageModel>(configuration, logger, serviceProvider,
            TopicConstant.UserRoadmapGenCreated, "user-roadmap-gen-created-batch-group")
    {
        protected override async Task ProcessBatch(IEnumerable<string> messages, IServiceProvider serviceProvider)
        {
            ...
            var producer = scopedProvider.GetRequiredService<IProducerService>();
            var sender = scopedProvider.GetRequiredService<ISender>();
            
            const int maxRetries = DataConstaints.MaxRetries;
            ...
            while (retryMessages.Any())
            {
                ...
                foreach (var retryMessage in retryBatch)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            logger.LogWarning(DataConstaints.UserServiceLogFirst + $"Max retries exceeded for UserId: {retryMessage.UserId}. Sending to retry topic.");
