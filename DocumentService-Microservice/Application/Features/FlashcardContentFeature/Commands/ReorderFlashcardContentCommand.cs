    ...
    : IRequestHandler<ReorderFlashcardContentCommand, ResponseModel>
{
    public async Task<ResponseModel> Handle(ReorderFlashcardContentCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = await unitOfWork.FlashcardContentRepository.UpdateRank(flashcardContent);
			if (result is false)
			{
				await unitOfWork.RollbackTransactionAsync();
				return new ResponseModel(HttpStatusCode.InternalServerError, "Đã xảy ra lỗi trong quá trình cập nhật thứ hạng thẻ ghi nhớ");
			}
			var result2 = await unitOfWork.FlashcardContentRepository.UpdateRank(flashcardOldWithRank);
			if (result2 is false)
            {
				await unitOfWork.RollbackTransactionAsync();
				return new ResponseModel(HttpStatusCode.InternalServerError, "Đã xảy ra lỗi trong quá trình cập nhật thứ hạng thẻ ghi nhớ");
